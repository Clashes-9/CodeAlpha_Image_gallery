...
const IMAGES = [
  { title: "Canopy Drift",       category: "nature",       img: "images/nature-1.jpg" },
...
// Returns the inline style string for a tile's art layer: a real
// photo (cover-fit) if item.img is set, otherwise the generated
// gradient artwork.
function artworkStyle(index) {
  const item = IMAGES[index];
  if (item.img) {
    return `background-image:url('${item.img}'); background-size:cover; background-position:center;`;
  }
  return `background:${generateArtwork(index, item.category)};`;
}
...
function renderGallery() {
  ...
  IMAGES.forEach((item, index) => {
    ...
    tile.innerHTML = `
      <div class="tile__art" style="${artworkStyle(index)}"></div>
...
function showImage(index) {
  currentIndex = index;
  const item = IMAGES[index];
  lbArt.style.cssText = artworkStyle(index);
